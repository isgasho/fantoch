...
#[derive(Debug)]
pub struct Shared<K: Eq + Hash + 'static, V: 'static> {
    shared: DashMap<K, V>,
}
    ...
    K: Eq + Hash + Clone,
{
    ...
    pub fn get_or_all<'k, 'd, F>(
        &'d self,
        keys: &BTreeSet<&'k K>,
        refs: &mut Vec<(&'k K, ElementGuard<K, V>)>,
        value: F,
    ) where
        F: Fn() -> V + Copy,
    {
    ...
    pub fn insert(&self, key: K, value: V) -> Option<V> {
        todo!("insert now returns a bool");
        // self.shared.insert(key, value)
    }

    pub fn remove(&self, key: &K) -> Option<(K, V)> {
        todo!("remove now returns a bool");
